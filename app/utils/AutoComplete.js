...
Ext.define('frontapp.utils.AutoComplete', {
	...
	config: {
		...
		defaults: {
			...
			resultsDisplayItemTpl : '{formatted_address}',
			resultsDisplayEmptyText : 'Aucun résultat',
			resultsDisplayLoadingText : 'Recherche...',
			resultsDisplayOverlayZIndex : 50,	
			resultsDisplayOverlayResultsHeight : 50,
			resultsDisplayOverlayWidth : '90%',
			resultsDisplayOverlayHeight : '80%',
			resultsDisplayOverlayInputPlaceHolder : 'Saisissez votre adresse...',
	...
	initialize: function() {
        ...
        if(me.config.config.autoCompleteRequest.requestType == 'gmap') {
            /*var map = new google.maps.Map(document.getElementById('map-canvas'), {
                mapTypeId: google.maps.MapTypeId.ROADMAP
            });*/


        }else if(me.config.config.autoCompleteRequest.requestType == 'remote') {
            ...
            me.config.config.autoCompleteRequest.proxy.reader = {
                type: 'json',
                rootProperty: 'results'
            };
            me.config.config.autoCompleteRequest.proxy.pageParam= false; //to remove param "page"
            me.config.config.autoCompleteRequest.proxy.startParam= false; //to remove param "start"
            me.config.config.autoCompleteRequest.proxy.limitParam= false; //to remove param "limit"
            me.config.config.autoCompleteRequest.proxy.noCache= false; //to remove param "_dc"
        ...
        this.on('painted',function () {
            console.log ('génération des services googles');
            /*me._map = new google.maps.Map(document.getElementById('map-canvas'), {
                mapTypeId: google.maps.MapTypeId.ROADMAP
            });
            console.log ('génération des services googles',me._map);
            me._infowindow = new google.maps.InfoWindow();
            me._service = new google.maps.places.PlacesService(me._map);*/
		...
		var doSearchWithTimeout = function() {
			...
			if(me.getValue(true).length >= me.config.config.resultsDisplay.qMinLength) {
				searchTimeout = setTimeout(function() {
                    ...
                    if(me.config.config.autoCompleteRequest.requestType == 'gmap') {
                        console.log(me.getValue(true));
                        var request = {
                            location: 'bouillargues',//me.getValue(true),
                            radius: 500,
                            types: ['store']
                        };
                         me._service.nearbySearch(request, function (results, status) {
                            console.log(results);
